...
export class FichaStaffComponent implements OnInit {
  ...
  // ===== CROPPER DE IMAGEM (Foto Perfil 3:4 / Avatar 1:1) =====
  ...
  public cropViewportWidth: number = 225;
  public cropViewportHeight: number = 300;
  ...
  onDateChange(date: NgbDateStruct | null) {
    console.log("FichaStaffComponent | onDateChange | date:", date);
    if (date) {
      ...
      console.log("FichaStaffComponent | onDateChange | dataNascimentoDisplay:", this.dataNascimentoDisplay);
    } else {
      this.dataNascimentoDisplay = '';
    }
  ...
  private uploadCroppedBlob(blob: Blob) {
    ...
    const nomefoto = isAvatarUpload
      ? (this.staff.id.toString() + '_avatar_staff')
      : (this.staff.id.toString() + '_staff');
